...
void Renderer::initVulkan(){
  ...
  disp = vkb_device.make_table();

  auto graphics_queue_ret = vkb_device.get_queue(vkb::QueueType::graphics);
  if (!graphics_queue_ret) {
    std::cerr << "Failed to get graphics queue. Error: " << graphics_queue_ret.error().message() << std::endl;
  }
...
void Renderer::cleanup(){
  ...
  disp.destroyPipeline(graphicsPipeline, nullptr);
  disp.destroyPipelineLayout(pipelineLayout, nullptr);
  disp.destroyRenderPass(renderPass, nullptr);
...
void Renderer::createRenderPass() {
  ...
  depthAttachment.format = vkb_swapchain.image_format;
  ...
  colorAttachmentResolve.format = vkb_swapchain.image_format;
...
void Renderer::createGraphicsPipeline() {
  ...
  VkShaderModule vertShaderModule = createShaderModule(this->disp, vertShaderCode);
  VkShaderModule fragShaderModule = createShaderModule(this->disp, fragShaderCode);
...
void Renderer::createFramebuffers(){
  ...
  for(size_t i = 0; i < swapchain_image_views.size(); i++){
    std::array<VkImageView, 2> attachments = {swapchain_image_views[i], depthImageView};
    ...
    framebufferInfo.width = vkb_swapchain.extent.width;
    framebufferInfo.height = vkb_swapchain.extent.height;
...
VkShaderModule Renderer::createShaderModule(vkb::DispatchTable disp, const std::vector<char>& code) {
